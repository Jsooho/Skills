...
void		Fbxobject::LoadMaterial(Mtrl* pMtrl)
{
	...
	if (pMtrl->m_pSubMtrl.size() > 0)
	{
		...
		{
			Mtrl* pSubMtrl = pMtrl->m_pSubMtrl[iSub];
	...
	for (int iObj = 0; iObj < m_pMeshList.size(); iObj++)
	{
		...
		if (pMesh->m_pSubMesh.size() > 0)
		{
			for (int iSub = 0; iSub < pMesh->m_pSubMesh.size(); iSub++)
			{
				...
				pMesh->m_pSubMesh[iSub]->SetMatrix(&m_cbData.matrixWorld,
		...
		else
		{
			...
			pMesh->SetMatrix(&m_cbData.matrixWorld, &m_cbData.matrixView, &m_cbData.matrixProjection);
