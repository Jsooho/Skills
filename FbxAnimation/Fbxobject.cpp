		...
		for (int iSub = 0; iSub < pMtrl->m_SubMtrl.size(); iSub++)
		{
			...
			if (prop.IsValid())
			{
				...
				for (int iTex = 0; iTex < iTexCount; iTex++)
				{
					...
					pSubMtrl->m_Texture.m_csName = TBASIS::g_szDataPath;
					pSubMtrl->m_Texture.m_csName += L"object/";
					pSubMtrl->m_Texture.m_csName += TBASIS::mtw(szFileName);
					pSubMtrl->m_Texture.Load(pSubMtrl->m_Texture.m_csName);
				}
			}
		}
	}
	...
	for (int iMesh = 0; iMesh < m_pMeshList.size(); iMesh++)
	{
		...
		if (pMesh->m_pSubMesh.size() > 0)
		{
			for (int iSubMesh = 0; iSubMesh < m_pMeshList[iMesh]->m_pSubMesh.size(); iSubMesh++)
			{
				...
				pSubMesh->Create(pSubMesh->m_pContext,L"", L"", L"FbxShader.hlsl");
			}
		}
		else
		{
	...
	for (int iObj = 0; iObj < m_pMeshList.size(); iObj++)
	{
		...
		if (pMesh->m_pSubMesh.size() > 0)
		{
			for (int iSub = 0; iSub < pMesh->m_pSubMesh.size(); iSub++)
			{
				...
				pContext->PSSetShaderResources(0, 1, &pSubMtrl->m_Texture.m_pSRV);
