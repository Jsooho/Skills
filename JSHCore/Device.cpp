...
HRESULT Device::SetDepthStencilView()
{
	...
	hr = m_DefaultDS.CreateDepthStencilView(Desc.BufferDesc.Width, Desc.BufferDesc.Height);
	return hr;
}

bool Device::SetDevice()
{
	HRESULT hr;
...
HRESULT Device::CreateSwapChain(HWND hWnd, UINT iWidth, UINT iHeight)
{
	...
	g_iClientWidth = sd.BufferDesc.Width;
	g_iClientHeight = sd.BufferDesc.Height;
...
HRESULT Device::SetRenderTargetView()
{
	...
	if (FAILED(hr = m_pSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&pBackBuffer), hr))
	{
		return hr;
	}
