#include "DxWrite.h"
HRESULT DxWrite::CreateDeviceResources(IDXGISurface1* pSurface)
{
    HRESULT hr = S_OK;
    ...
    hr = m_pd2dFactory->CreateDxgiSurfaceRenderTarget( pSurface, &props, &m_pRT);
    if (FAILED(hr)) return hr;

    m_pRT->CreateSolidColorBrush( D2D1::ColorF(D2D1::ColorF::Yellow),  &m_pTextBrush);
    if (FAILED(hr)) return hr;
    return S_OK;
}
bool DxWrite::Init()
{
    ...
    if (FAILED(hr)) return false;
     ...
     hr = m_pdWriteFactory->CreateTextFormat( L"±Ã¼­",NULL, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL,DWRITE_FONT_STRETCH_NORMAL, 20,L"ko-kr", &m_pTextFormat);
