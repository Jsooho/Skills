...
HRESULT Mesh::CreateVertexLayout()
{
	...
	hr = g_pd3dDevice->CreateInputLayout(layout, _countof(layout), m_pVSBlob->GetBufferPointer(), m_pVSBlob->GetBufferSize(), &m_pVertexLayout);
	if (FAILED(hr)) return hr;
...
bool Mesh::PreRender(ID3D11DeviceContext* pContext)
{
	...
	ID3D11Buffer* vb[2] = { m_VertexBuffer, m_pIWVertrexBuffer };
